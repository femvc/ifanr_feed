...
hui.Validator = {
  ...
  getControl: function(control) {
    var ctr = control && control.getMain ?
      control.getMain() :
      (control && control.main ? document.getElementById(control.main) : control)
    return ctr
  },
  showError: function(control, noticeText) {
    if (hui.Message) hui.Message.error(noticeText)
    // else hui.Validator.showNoticeDom(control, noticeText, 'errorClass')
  },
  ...
  showNoticeDom: function(control, noticeText, className) {
    ...
    hui.removeClass(father, me.config['okClass'] + ' ' + me.config['errorClass'] + ' ' + me.config['waitingClass'])
    hui.addClass(father, me.config[className])
  ...
  cancelNotice: function(control) {
    ...
    hui.removeClass(father, me.config['errorClass'] + ' ' + me.config['okClass'] + ' ' + me.config['waitingClass'])
  ...
  getTextEl: function(input) {
    var me = this,
      ...
      control = input
    input = me.getControl(control)
    elem = document.getElementById(input.id + '_' + me.config['textSuffix'])
  ...
  getValidateElem: function(input) {
    var me = this,
      ...
      control = input
    input = me.getControl(control)
    elem = document.getElementById(input.id + '_' + me.config['suffix'])
    return elem
  },
  /**
   * @name 获取提示元素
  ...
  getSegElement: function(control, segId) {
    var elem = document.getElementById(segId)
    return elem
  },
  /**
   * @name 验证规则
  ...
  applyRule: function(control, ruleName, show_error) {
    ...
    // return ['ERROR_LENGTH_RANGE', min_length, max_length]
      ...
      errorText = hui.format(me.errorMsg[error[0]], error.splice(1, error.length))
    ...
    if (show_error !== 'not_show' && show_error !== 'no_show' && show_error !== 'noshow') {
      // return 0 or 'SUCCESS'
...
hui.importCssString([
  ...
  '.validate_error .validate_text {border: 0px solid red;}',
