 ...
 * @example 
 Draggable('targetElementId',
    ...
    move: function(){
      ...
      this.moving = true
      //alert('move')
      //this.elem.style.left = this.oldPosition.left+'px'
      //this.elem.style.top = this.oldPosition.left +'px'
...
var Draggable = (function() {
  ...
  function _drag(elem, opt) {
    ...
    me.elem = (typeof elem == 'string') ? document.getElementById(elem) : elem //被拖动节点
    me.onstart = opt.start || function () {} //
    me.onmove = opt.move || function () {}
    me.onend = opt.end || function () {}
    me.click = opt.click || function () {}
    me.revert = opt.revert === undefined || opt.revert ? true : false
  ...
  _drag.prototype = {
      ...
      elem['on' + me.startEvent] = me.bind(function(e) { //绑定节点的 [鼠标按下/触摸开始] 事件
        ...
        if (this.onstart(e) !== false) {
          ...
          this.relativePosition = {
            left: this.elem.offsetLeft - this.oldPosition.left,
            top: this.elem.offsetTop - this.oldPosition.top
          }
          this.relativeCursor = {
    ...
    appendTo: function(drop, point) {
      var me = this
      var newParentOffset = me.cumulativeOffset(drop)
      // var oldParentOffset = me.cumulativeOffset(me.elem.parentNode)
      var num
      var taskItems
      var frontNum
      var child

      taskItems = hui.c('task', drop)
...
var Droppables = {
  ...
  show: function(point, drag) {
    ...
    for (i = 0, len = affected.length; i < len; i++) {
      drop = affected[i]

      if (drop.onHover) drop.onHover(drag, drop, point)
      else Droppables.onHover(drag, drop, point)
    }
  },
  ...
  onHover: function(drag, drop, point) {
    // var elem = drag.elem
    // var oldParentNode = elem.parentNode

    drag.appendTo(drop, point)
  }
}

// !!! global.Draggable = ...
if (typeof window != 'undefined') {
  window.Droppables = Droppables
}
if (typeof global != 'undefined') {
  global.Droppables = Droppables
}