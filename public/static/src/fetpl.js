...
fetpl.parseFilePath = function (dir, sub, ext) {
  ...
  return path.join(path.dirname(dir), String(sub).endWith(String(ext)) ? sub : sub + ext)
}
...
fetpl.render = function (filePath, data, callback) { // define the template engine  
  ...
  new Promise(function (resolve) {
    ...
    var out = ''
    try {
      // 注：fetpl.template() 和 fetpl.render() 和 fetpl._render()
      var fn = fetpl.template(fetpl.tmplist[filePath], null)
      out = fn(data)
    } catch (e) {
      // fn.toString().split('\n')[40]
      var row = (e.stack || '').match(/<anonymous>\:\d+/ig)
      if (row) {
        var str = fn.toString().split('\n')
        var line = parseInt(row[0].replace('<anonymous>:', ''))
        if (str[line - 3]) out += fetpl.helper.decodeURI(str[line - 3]).replace(/[ \t\n\r]+/g, ' ') + '\n'
        if (str[line - 2]) out += fetpl.helper.decodeURI(str[line - 2]).replace(/[ \t\n\r]+/g, ' ') + '\n'
        if (str[line - 1]) out += fetpl.helper.decodeURI(str[line - 1]).replace(/[ \t\n\r]+/g, ' ') + '\n'
        if (str[line + 0]) out += fetpl.helper.decodeURI(str[line + 0]).replace(/[ \t\n\r]+/g, ' ') + '\n'
        if (str[line + 1]) out += fetpl.helper.decodeURI(str[line + 1]).replace(/[ \t\n\r]+/g, ' ') + '\n'
        if (str[line + 2]) out += fetpl.helper.decodeURI(str[line + 2]).replace(/[ \t\n\r]+/g, ' ') + '\n'
        if (str[line + 3]) out += fetpl.helper.decodeURI(str[line + 3]).replace(/[ \t\n\r]+/g, ' ') + '\n'
      }
      out = out = '<style>body{padding:20px;}</style><h3>' + e.message + '</h3><pre style="font-size:16px;line-height:1.5em;">' + 
        fetpl.helper.encode(out) + '</pre>'
    }
    callback(null, out)
  })
}
...
if (typeof global !== 'undefined' && typeof exports !== 'undefined') {
  // 注：下面这行不能注释掉，在fetpl.render中生成的模板解析函数中会用到global.fetpl
  global.fetpl = fetpl
  
  exports.render = fetpl.render
  exports.helper = fetpl.helper
...
fetpl.helper.encodeURI = function (str, isDecode) {
  ...
  var i, l, fr = '%|&|;|=|+|<|>|,|"|\'|#|/|\\|\n|\r|\t'.split('|'),
    to = '%25|%26|%3B|%3D|%2B|%3C|%3E|%2C|%22|%27|%23|%2F|%5C|%0A|%0D|%09'.split('|')
...
if (!String.prototype.startWith) String.prototype.startWith = String.prototype.startsWith
...
if (!String.prototype.endWith) String.prototype.endWith = String.prototype.endsWith
